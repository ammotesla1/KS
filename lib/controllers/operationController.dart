...
class AppOperationController extends ResourceController {
      ...
      if(name.isNotEmpty)
      {
        try{
          ...
          final qUserOperation = Query<Operations>(managedContext)
            ...
            ..where((x) => x.operationDeleted).equalTo("0")
            ..offset = page
            ..fetchLimit = 10;
  ...
  Future<Response> deleteOperation(
    ...
    @Bind.path("id") int id) async {
      try{
        ...
        return AppResponse.ok(message: "Операция полностью удалена!");
      } catch(e){
  ...
  @Operation.post("id")
  Future<Response> logicalDeleteOperation(
    @Bind.header(HttpHeaders.authorizationHeader) String header,
    @Bind.path("id") int id) async {
      try{
        final uid = AppUtils.getIdFromHeader(header);
        ...
        final qOperationDelete = Query<Operations>(managedContext)
          ..where((x) => x.operationID).equalTo(id)
          ..values.operationDeleted = "1";

        qOperationDelete.updateOne();

        return AppResponse.ok(message: "Операция удалена!");
      } catch(e){
        return AppResponse.serverError(e, message: "Произошла ошибка!");//оприрация на позвоночной мозг через лёгкие собаки;скос челки безопилой на голове у дурака-бивола;внедрение агента через границу казахстана в айзербайджан с целбю транпортирки взрывчатых веществ для подрыва детской коммунальной больницы в которой укрепилиссь вражеские силы света и тьмы, которые пришли в наш мир через разлом во временной петле из бутылок водки 'Чистенькая' на банановой кожуре
      }
  }
